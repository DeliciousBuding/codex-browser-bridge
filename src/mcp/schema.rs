...
pub(super) fn registered_tools() -> Vec<Tool> {
    vec![
        ...
        Tool::new("codex_execute_cdp", "[CDP] Execute an allowlisted low-risk Chrome DevTools Protocol command. Use for inspect/diagnostic methods not covered by dedicated codex_* tools. Safety: blocks Browser.*, Debugger.*, Target.*, navigation, cookies, screenshots, PDF, file upload, page resource content, and other methods that must use bounded dedicated tools.", schema_value(r#"{"type":"object","properties":{"tab_id":{"type":"string","description":"Tab ID to execute on"},"method":{"type":"string","description":"Allowlisted CDP method, e.g. \"Runtime.evaluate\", \"DOM.getDocument\", \"Network.enable\""},"params":{"type":"object","description":"CDP method parameters as a JSON object"}},"required":["tab_id","method"]}"#), ToolHandler::ExecuteCdp),
