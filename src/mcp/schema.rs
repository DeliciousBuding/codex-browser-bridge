...
pub(super) fn registered_tools() -> Vec<Tool> {
    vec![
        ...
        Tool::new("codex_print_pdf", "[Page] Render the current page to PDF via Page.printToPDF (A4, backgrounds on). Returns a size summary — the base64 PDF is large and is not embedded in the response. Use codex_evaluate or a dedicated capture if you need the bytes.", schema_value(r#"{"type":"object","properties":{"tab_id":{"type":"string"}},"required":["tab_id"]}"#), ToolHandler::PrintPdf),
        Tool::new("codex_storage", "[Network] Read or write localStorage. action='get' returns the value (or null); action='set' writes it. Useful for login state, tokens, and SPA app state stored client-side.", schema_value(r#"{"type":"object","properties":{"tab_id":{"type":"string"},"key":{"type":"string","description":"localStorage key"},"value":{"type":"string","description":"Value to set (action=set only)"},"action":{"type":"string","enum":["get","set"],"description":"Read or write. Default: get"}},"required":["tab_id","key"]}"#), ToolHandler::Storage),
        ...
        Tool::new("codex_network_monitor", "[Network] Capture Network.* events (requestWillBeSent, responseReceived, etc.) for a duration. Enables the Network domain, collects events, then disables. Returns raw event params — can be large, use a short duration_ms. Useful for debugging API calls, inspecting XHR/fetch traffic, or reverse-engineering endpoints.", schema_value(r#"{"type":"object","properties":{"tab_id":{"type":"string"},"duration_ms":{"type":"integer","description":"Capture window in ms. Default 5000."}},"required":["tab_id"]}"#), ToolHandler::NetworkMonitor),
...
mod tests {
    ...
    fn registered_tools_keep_go_order() {
        ...
        assert_eq!(names.last(), Some(&"codex_console_logs"));
