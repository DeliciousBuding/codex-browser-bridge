...
pub(super) fn registered_tools() -> Vec<Tool> {
    vec![
        ...
        Tool::new("codex_execute_cdp", "[CDP] Execute an explicitly allowlisted low-risk Chrome DevTools Protocol command. Use for inspect/diagnostic methods not covered by dedicated codex_* tools. Safety: raw domains are not wildcard-open; navigation, cookies, screenshots, PDF, file upload, page resource content, and destructive methods must use bounded dedicated tools. Text output is bounded by CODEX_BRIDGE_MAX_TEXT_BYTES.", schema_value(r#"{"type":"object","properties":{"tab_id":{"type":"string","description":"Tab ID to execute on"},"method":{"type":"string","description":"Explicitly allowlisted CDP method, e.g. \"Runtime.evaluate\", \"DOM.getDocument\", \"Network.enable\""},"params":{"type":"object","description":"CDP method parameters as a JSON object"}},"required":["tab_id","method"]}"#), ToolHandler::ExecuteCdp),
