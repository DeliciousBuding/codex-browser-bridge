...
func (s *MCPServer) registerTools() {
	s.tools = []Tool{
		...
		{Name: "codex_screenshot", Description: "Capture a screenshot (returns base64 PNG)",
...
func (s *MCPServer) handleListTabs(_ json.RawMessage) (string, error) {
...
func (s *MCPServer) handleCreateTab(_ json.RawMessage) (string, error) {
	id, err := s.client.CreateTab()
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("Created tab: %s", id), nil
}

func (s *MCPServer) handleCloseTab(args json.RawMessage) (string, error) {
	var p struct {
		TabID string `json:"tab_id"`
	}
	json.Unmarshal(args, &p)
	err := s.client.CloseTab(p.TabID)
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("Closed tab %s", p.TabID), nil
}

func (s *MCPServer) handleUserTabs(_ json.RawMessage) (string, error) {
...
func (s *MCPServer) handleClaimTab(args json.RawMessage) (string, error) {
	var p struct {
		TabID string `json:"tab_id"`
	}
	json.Unmarshal(args, &p)
...
func (s *MCPServer) handleNavigate(args json.RawMessage) (string, error) {
	var p struct {
		TabID string `json:"tab_id"`
		URL   string `json:"url"`
	}
	json.Unmarshal(args, &p)
	err := s.client.Navigate(p.TabID, p.URL)
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("Navigated tab %s to %s", p.TabID, p.URL), nil
}

func (s *MCPServer) handleReload(args json.RawMessage) (string, error) {
	var p struct {
		TabID string `json:"tab_id"`
	}
	json.Unmarshal(args, &p)
	err := s.client.Reload(p.TabID)
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("Reloaded tab %s", p.TabID), nil
}

func (s *MCPServer) handleDOMSnapshot(args json.RawMessage) (string, error) {
	var p struct {
		TabID string `json:"tab_id"`
	}
	json.Unmarshal(args, &p)
...
func (s *MCPServer) handleScreenshot(args json.RawMessage) (string, error) {
	var p struct {
		TabID   string `json:"tab_id"`
...
func (s *MCPServer) handleClick(args json.RawMessage) (string, error) {
	var p struct {
		TabID    string `json:"tab_id"`
		Selector string `json:"selector"`
	}
	json.Unmarshal(args, &p)
	err := s.client.Click(p.TabID, p.Selector)
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("Clicked %s in tab %s", p.Selector, p.TabID), nil
}

func (s *MCPServer) handleFill(args json.RawMessage) (string, error) {
	var p struct {
		TabID    string `json:"tab_id"`
		Selector string `json:"selector"`
	...
	json.Unmarshal(args, &p)
	err := s.client.Fill(p.TabID, p.Selector, p.Value)
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("Filled %s in tab %s", p.Selector, p.TabID), nil
}

func (s *MCPServer) handleEvaluate(args json.RawMessage) (string, error) {
	var p struct {
		TabID      string `json:"tab_id"`
...
func (s *MCPServer) handleCUAClick(args json.RawMessage) (string, error) {
	var p struct {
		TabID string `json:"tab_id"`
		X     int    `json:"x"`
		Y     int    `json:"y"`
	...
	json.Unmarshal(args, &p)
	err := s.client.CUAClick(p.TabID, p.X, p.Y)
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("CUA click at (%d,%d) in tab %s", p.X, p.Y, p.TabID), nil
}

func (s *MCPServer) handleCUAType(args json.RawMessage) (string, error) {
	var p struct {
		TabID string `json:"tab_id"`
		Text  string `json:"text"`
	}
	json.Unmarshal(args, &p)
	err := s.client.CUAType(p.TabID, p.Text)
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("CUA typed text in tab %s", p.TabID), nil
}

func (s *MCPServer) handleCUAKeypress(args json.RawMessage) (string, error) {
	var p struct {
		TabID string   `json:"tab_id"`
		Keys  []string `json:"keys"`
	}
	json.Unmarshal(args, &p)
	err := s.client.CUAKeypress(p.TabID, p.Keys)
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("CUA keypress %v in tab %s", p.Keys, p.TabID), nil
}

func (s *MCPServer) handleCUAScroll(args json.RawMessage) (string, error) {
	var p struct {
		TabID   string `json:"tab_id"`
		X       int    `json:"x"`
		Y       int    `json:"y"`
	...
	json.Unmarshal(args, &p)
	err := s.client.CUAScroll(p.TabID, p.X, p.Y, p.ScrollX, p.ScrollY)
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("CUA scroll at (%d,%d) delta (%d,%d) in tab %s", p.X, p.Y, p.ScrollX, p.ScrollY, p.TabID), nil
}

func (s *MCPServer) handleGetVisibleDOM(args json.RawMessage) (string, error) {
	var p struct {
		TabID string `json:"tab_id"`
	}
	json.Unmarshal(args, &p)
...
func (s *MCPServer) handleDomClick(args json.RawMessage) (string, error) {
	var p struct {
		TabID  string `json:"tab_id"`
		NodeID string `json:"node_id"`
	}
	json.Unmarshal(args, &p)
	err := s.client.DomCUAClick(p.TabID, p.NodeID)
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("DOM click node %s in tab %s", p.NodeID, p.TabID), nil
}

func (s *MCPServer) handleNameSession(args json.RawMessage) (string, error) {
	var p struct {
		Name string `json:"name"`
	}
	json.Unmarshal(args, &p)
	err := s.client.NameSession(p.Name)
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("Session named: %s", p.Name), nil
}

func (s *MCPServer) handleFinalize(_ json.RawMessage) (string, error) {
	err := s.client.FinalizeTabs(nil)
	if err != nil {
		return "", err
	}
	return "Tabs finalized", nil
}

func (s *MCPServer) handleGetInfo(_ json.RawMessage) (string, error) {
