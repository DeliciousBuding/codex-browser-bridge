...
**Why?** Codex Desktop's `REDACTED` depends on `import.meta.REDACTED`, a privileged object injected by Codex's custom ESM loader. Claude Code can't access it. But the underlying protocol is simple (JSON-RPC 2.0 over length-prefixed frames), so this bridge reimplements it.
